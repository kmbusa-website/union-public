...
const PAST_PAPERS_CSV = path.join(process.cwd(), "public", "exams", "pastpapers.csv");
...
export const loadPastPapers = cache(async (): Promise<PastPaper[]> => {
  try {
    const csv = await readFile(PAST_PAPERS_CSV, "utf8");
    return parseCsv(csv);
  } catch {
    return [];
  }
...
export const loadPastPaperYears = cache(async (): Promise<number[]> => {
  const papers = await loadPastPapers();
  return Array.from(new Set(papers.map((paper) => paper.year))).sort((a, b) => b - a);
});

export const loadPastPapersByYear = cache(async (year: number): Promise<PastPaper[]> => {
  const papers = await loadPastPapers();
